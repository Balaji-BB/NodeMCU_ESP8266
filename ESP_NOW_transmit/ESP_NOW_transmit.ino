...
char msg_content[] = "Hello from Transmitter!";
uint8_t hand_shaken_cnt = 1;
char send_msg[30] = "";
char recv_msg[30] = "";
...
void setup() {
  ...
  esp_error = esp_now_send((uint8_t*)RECEIVER_MAC_ADDRESS, (uint8_t*)send_msg, size + len + 1);
...
void loop() {
  ...
  if (esp_now_rx_flag == true)
  {
    ...
    Serial.println("Received Message on Transmitter side : ");
    ...
    if (ch != NULL)
    {
      ...
      int size = snprintf((char*)&send_msg[len], sizeof(send_msg) - len, "%d", hand_shaken_cnt);
      Serial.println("Transmitting Message on Transmitter side: ");
...
void esp_now_initialization(void)
{
  Serial.println("ESP COMBO - TRANSMITTER");
  delay(1000);
  ...
  esp_error = esp_now_add_peer((uint8_t*)RECEIVER_MAC_ADDR, ESP_NOW_ROLE_CONTROLLER, 0, NULL, 0);
  ...
  esp_error = esp_now_add_peer((uint8_t*)RECEIVER_MAC_ADDRESS, ESP_NOW_ROLE_COMBO, 0, NULL, 0);
  (esp_error == 0) ? esp_success_display("PEER_ADDITION") : esp_error_display();
